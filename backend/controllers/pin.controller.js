...
export const interactionCheck = async (req, res) => {
  ...
  const pin = await Pin.findById(id).select('user'); // 핀 업로더의 _id 가져오기
  if (!pin) {
    return res.status(404).json({ message: 'Pin not found' });
  }
  
  const uploaderId = pin.user.toString(); // 업로더의 user _id

  const likeCount = await Like.countDocuments({ pin: id });

  if (!token) {
    return res.status(200).json({ likeCount, isLiked: false, isSaved: false, uploaderId });
  }
  ...
  jwt.verify(token, process.env.JWT_SECRET, async (err, payload) => {
    if (err) {
      return res
        .status(200)
        .json({ likeCount, isLiked: false, isSaved: false, uploaderId });
    }
...
export const deletePin = async (req, res) => {
  try {
    ...
    if (pin.user.toString() !== req.userId) {
      return res.status(403).json({ message: "You can only delete your own pin" });
    }
